...
def actions_for_generating_keys(symmetric: Symmetric, asymmetric: Asymmetric, settings: dict)->None:
    ...
    symmetric.serialize_key(settings['sym_key'])
    asymmetric.generate_and_serialize_key_pair(settings['private_key'],settings['public_key'])
...
def actions_to_decrypt_symmetric_key(asymmetric: Asymmetric, settings: dict)->None:
    esym_key = read_bytes(settings['encrypted_sym'])
    sym_key = asymmetric.process_symmetric_key(esym_key, settings['private_key'],'decrypt')            
    write_bytes_to_txt(sym_key, settings['decrypted_sym'])
...
def actions_for_encrypting_text(symmetric: Symmetric, settings: dict)->None:
    sym_key = symmetric.deserialize_key(settings['sym_key'])
    text = read_from_file(settings['original_text'])
    e_text = symmetric.process_text(text, sym_key, 'encrypt')
    write_bytes_to_txt(e_text, settings['encrypted_text'])

    print(f'The text has been successfully encrypted')


def actions_for_decrypting_text(symmetric: Symmetric, settings: dict)->None:
    dec_sym_key = symmetric.deserialize_key(settings['decrypted_sym'])
    text = read_bytes(settings['encrypted_text'])
    dec_text = symmetric.process_text(text, dec_sym_key,'decrypt')
    save_to_file(settings['decrypted_text'], dec_text)
...
def main():
    ...
    settings = read_from_json_file('settings.json')
    match args:
